...
public class Graphmaster {
	private static final Log log = LogFactory.getLog(Graphmaster.class);
	public Bot bot;
	...
	/**
	 * add a Path to the graph from a given node. Shortcuts: Replace all
	 * instances of paths "<THAT> * <TOPIC> *" with a direct link to the
	 * matching category
	 *
	 * @param node
	...
	/**
	 * Given an input pattern, that pattern and topic pattern, find the leaf
	 * node associated with this path.
	 *
	 * @param input
	 *            input pattern
	 * @param that
	...
	final Nodemapper match(Path path, String inputThatTopic) {
		try {
			...
			String matchTrace = "";
			Nodemapper n = match(path, root, inputThatTopic, starState, 0, inputStars, thatStars, topicStars,
					matchTrace);
			if (n != null) {
			...
			// if (!n.category.getPattern().contains("*")) log.info("adding
			// match "+inputThatTopic);
			if (n != null)
	...
	/**
	 * Depth-first search of the graph for a matching leaf node. At each node,
	 * the order of search is 1. $WORD (high priority exact word match) 2. #
	 * wildcard (zero or more word match) 3. _ wildcard (one or more words
	 * match) 4. WORD (exact word match) 5. {@code <set></set>} (AIML Set match)
	 * 6. shortcut (graph shortcut when that pattern = * and topic pattern = *)
	 * 7. ^ wildcard (zero or more words match) 8. * wildcard (one or more words
	 * match)
	 *
	 * @param path
	 ...
	 * @param starState
	 *            tells whether wildcards are in input pattern, that pattern or
	 *            topic pattern
	 * @param starIndex
	 *            index of wildcard
	 * @param inputStars
	 ...
	 * @return matching leaf node or null if no match is found
	 */
	final Nodemapper match(Path path, Nodemapper node, String inputThatTopic, String starState, int starIndex,
		...
		else if ((matchedNode = dollarMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = sharpMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = underMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = wordMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = setMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = shortCutMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = caretMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else if ((matchedNode = starMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars,
				topicStars, matchTrace)) != null)
			return matchedNode;
		else {
			return null;
	...
	final Nodemapper wordMatch(Path path, Nodemapper node, String inputThatTopic, String starState, int starIndex,
		...
		try {
			...
			if (path != null && NodemapperOperator.containsKey(node, uword)
					&& (matchedNode = match(path.next, NodemapperOperator.get(node, uword), inputThatTopic, starState,
	...
	final Nodemapper zeroMatch(Path path, Nodemapper node, String input, String starState, int starIndex,
		...
		if (path != null && NodemapperOperator.containsKey(node, wildcard)) {
			setStars(bot.properties.get(MagicStrings.null_star), starIndex, starState, inputStars, thatStars,
					topicStars);
			Nodemapper nextNode = NodemapperOperator.get(node, wildcard);
			return match(path, nextNode, input, starState, starIndex + 1, inputStars, thatStars, topicStars,
					matchTrace);
		} else {
			...
			if (path != null && NodemapperOperator.containsKey(node, wildcard)) {
				...
				if (NodemapperOperator.isLeaf(nextNode) && !nextNode.shortCut) {
					...
					for (path = pathStart; path != null && !currentWord.equals("<THAT>")
						...
						matchTrace += "[" + wildcard + "," + path.word + "]";
						if ((matchedNode = match(path, nextNode, input, starState, starIndex + 1, inputStars, thatStars,
								topicStars, matchTrace)) != null) {
							setStars(starWords, starIndex, starState, inputStars, thatStars, topicStars);
							return matchedNode;
						} else {
	...
	final Nodemapper setMatch(Path path, Nodemapper node, String input, String starState, int starIndex,
		...
		for (String setName : node.sets) {
			...
			for (Path qath = path.next; qath != null && !currentWord.equals("<THAT>") && !currentWord.equals("<TOPIC>")
					&& length <= aimlSet.maxLength; qath = qath.next) {
				...
				if (aimlSet.contains(phrase) && (matchedNode = match(qath, nextNode, input, starState, starIndex + 1,
						inputStars, thatStars, topicStars, matchTrace)) != null) {
					setStars(starWords, starIndex, starState, inputStars, thatStars, topicStars);
					// log.info("setMatch found "+phrase+" in "+setName);
					return matchedNode;
				}
				// else if (qath.word.equals("<THAT>") ||
				// qath.word.equals("<TOPIC>")) return null;
				else {
					length = length + 1;
	...
	public void setStars(String starWords, int starIndex, String starState, String[] inputStars, String[] thatStars,
			String[] topicStars) {
	...
	void printgraph(Nodemapper node, String partial) {
		if (node == null)
			...
			if (NodemapperOperator.isLeaf(node) || node.shortCut) {
				...
				if (node.shortCut)
					log.info(partial + "(" + NodemapperOperator.size(node) + "[" + node.key + "," + node.value
							+ "])--<THAT>-->X(1)--*-->X(1)--<TOPIC>-->X(1)--*-->" + template + "...");
				else
					log.info(partial + "(" + NodemapperOperator.size(node) + "[" + node.key + "," + node.value + "]) "
							+ template + "...");
			}
			for (String key : NodemapperOperator.keySet(node)) {
				...
				printgraph(NodemapperOperator.get(node, key),
						partial + "(" + NodemapperOperator.size(node) + "[" + node.height + "])--" + key + "-->");
			}
		}
	}
		...
		if (node == null)
			...
			if (NodemapperOperator.isLeaf(node) || node.shortCut) {
				if (node.category != null)
					categories.add(node.category); // node.category == null when
	...
	public void nodeStats() {
		...
		naryCnt = 0;
		nodeStatsGraph(root);
		resultNote = nodeCnt + " nodes " + singletonCnt + " singletons " + leafCnt + " leaves " + shortCutCnt
				+ " shortcuts " + naryCnt + " n-ary " + nodeSize + " branches " + (float) nodeSize / (float) nodeCnt
				+ " average branching ";
		log.info(resultNote);
	}
