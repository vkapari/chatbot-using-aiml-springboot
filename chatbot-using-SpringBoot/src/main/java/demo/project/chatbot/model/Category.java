...
public class Category {
	...
	public void addMatch(String input) {
		if (matches == null) {
			String setName = this.inputThatTopic().replace("*", "STAR").replace("_", "UNDERSCORE").replace(" ", "-")
					.replace("<THAT>", "THAT").replace("<TOPIC>", "TOPIC");
	...
	public static Category IFToCategory(String IF) {
		...
		// log.info("Read: "+split);
		return new Category(Integer.parseInt(split[0]), split[1], split[2], split[3], lineToTemplate(split[4]),
				split[5]);
	}
	...
	public static String categoryToAIML(Category category) {
		...
		try {
			...
			result = topicStart + "<category><pattern>" + category.getPattern() + "</pattern>" + thatStatement + NL
					+ "<template>" + category.getTemplate() + "</template>" + NL + "</category>" + topicEnd;
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return result;
	...
	public boolean validPatternForm(String pattern) {
		...
		for (int i = 0; i < words.length; i++) {
			 ...
			 * word.equals("*") || word.equals("_"))) { log.info(
			 * "Invalid pattern word "+word); return false; }
			 */
		}
		return true;
	}
	...
	public Category(int activationCnt, String pattern, String that, String topic, String template, String filename) {
		...
		this.template = template.replace("& ", " and "); // XML parser treats &
															// badly
		this.filename = filename;
	...
	public Category(int activationCnt, String patternThatTopic, String template, String filename) {
		this(activationCnt, patternThatTopic.substring(0, patternThatTopic.indexOf("<THAT>")),
				patternThatTopic.substring(patternThatTopic.indexOf("<THAT>") + "<THAT>".length(),
						patternThatTopic.indexOf("<TOPIC>")),
				patternThatTopic.substring(patternThatTopic.indexOf("<TOPIC>") + "<TOPIC>".length(),
						patternThatTopic.length()),
				template, filename);
	}
	...
	/**
	 * compare two categories for sorting purposes based on alphabetical order
	 * of patterns
	 */
	...
	/**
	 * compare two categories for sorting purposes based on category index
	 * number
	 */
