...
public class AIMLMap extends HashMap<String, String> {
	private static final Log log = LogFactory.getLog(AIMLMap.class);
			...
			while ((strLine = br.readLine()) != null && strLine.length() > 0) {
				...
				if (splitLine.length >= 2) {
					...
					if (strLine.startsWith(MagicStrings.remote_map_key)) {
						...
						// assume domain element is already normalized for
						// speedier load
	...
	public void readAIMLMap(Bot bot) {
		...
		try {
			...
			if (file.exists()) {
				...
				log.info(MagicStrings.maps_path + "/" + mapName + ".txt not found");
