...
import demo.project.chatbot.utils.DomUtils;
import demo.project.chatbot.utils.IOUtils;

/**
 * The core AIML parser and interpreter. Implements the AIML 2.0 specification
 * as described in AIML 2.0 Working Draft document
 * https://docs.google.com/document/d/1wNT25hJRyupcG51aO89UcQEiG-
 * HkXRXusukADpFnDs4/pub
 */
public class AIMLProcessor {
	private static final Log log = LogFactory.getLog(AIMLProcessor.class);
	/**
	 ...
	 * @param topic
	 *            value of topic in case this category is wrapped in a
	 *            <topic> tag
	 * @param aimlFile
	...
	public static ArrayList<Category> AIMLToCategories(String directory, String aimlFile) {
		try {
			...
			Node root = DomUtils.parseFile(directory + "/" + aimlFile); // <aiml>
																		// tag
	 ...
	 * http://stackoverflow.com/questions/1892765/capitalize-first-char-of-each-
	 * word-in-a-string-java
	 *
	 * @param string
	 ...
	 * requesting process. However if that markup contains AIML tags, those tags
	...
	private static String srai(Node node, ParseState ps) {
		...
		try {
			...
			String topic = ps.chatSession.predicates.get("topic"); // the that
																	// stays the
																	// same, but
																	// the topic
																	// may have
			...
			if (MagicBooleans.trace_mode) {
				log.info(trace_count + ". <srai>" + result + "</srai> from " + ps.leaf.category.inputThatTopic()
						+ " topic=" + topic + ") ");
				trace_count++;
			}
			...
			response = evalTemplate(leaf.category.getTemplate(),
					new ParseState(ps.depth + 1, ps.chatSession, ps.input, ps.that, topic, leaf));
			// log.info("That="+that);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	/**
	 * in AIML 2.0, an attribute value can be specified by either an XML
	 * attribute value or a subtag of the same name. This function tries to read
	 * the value from the XML attribute first, then tries to look for the
	 * subtag.
	 *
	 * @param node
	 *            current parse node.
	...
	private static String getAttributeOrTagValue(Node node, ParseState ps, String attributeName) { // AIML
																									// 2.0
		...
		if (m == null) {
			...
			for (int i = 0; i < childList.getLength(); i++) {
				Node child = childList.item(i);
				// log.info("getAttributeOrTagValue child =
				// "+child.getNodeName());
				if (child.getNodeName().equals(attributeName)) {
					result = evalTagContent(child, ps, null);
					// log.info("getAttributeOrTagValue result from child =
					// "+result);
				}
			}
		} else {
	...
	/**
	 * access external web service for response implements <sraix></sraix> and
	 * its attribute variations.
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	...
	private static String map(Node node, ParseState ps) {
		...
		if (mapName == null)
			result = "<map>" + contents + "</map>"; // this is an OOB map tag
													// (no attribute)
		else {
	...
	/**
	 * set the value of an AIML predicate. Implements
	 * <set name="predicate"></set> and <set var="varname"></set>
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	...
	/**
	 * get the value of an AIML predicate. implements
	 * <get name="predicate"></get> and <get var="varname"></get>
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	...
	/**
	 * return the value of a bot property. implements {{{@code 
	 * <bot name="property"/>}}}
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	 *            AIML parse state
	 * @return the bot property or a string indicating the property was not
	 *         found.
	 */
	...
	private static String date(Node node, ParseState ps) {
		...
		String jformat = getAttributeOrTagValue(node, ps, "jformat"); // AIML
																		// 2.0
	...
	/**
	 * get the value of an index attribute and return it as an integer. if it is
	 * not recognized as an integer, return 0
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	...
	/**
	 * implements <topicstar/> and <topicstar index="N"/> returns the value of
	 * input words matching the Nth wildcard (or AIML Set) in a topic pattern.
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	...
	/**
	 * implements {@code <system>} tag. Evaluate the contents, and try to
	 * execute the result as a command in the underlying OS shell. Read back and
	 * return the result of this command.
	 *
	 * The timeout parameter allows the botmaster to set a timeout in ms, so
	 * that the <system></system> command returns eventually.
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	 *            AIML parse state
	 * @return the result of executing the system command or a string indicating
	 *         the command failed.
	 */
	...
	/**
	 * Transform a string of words (separtaed by spaces) into a string of
	 * individual characters (separated by spaces). Explode "ABC DEF" =
	 * "A B C D E F".
	 *
	 * @param node
	 *            current XML parse node
	...
	/**
	 * apply the AIML denormalization pre-processor to the evaluated tag
	 * contenst. implements {@code <normalize>} tag.
	 *
	 * @param node
	 *            current XML parse node
	 * @param ps
	...
	/**
	 * evaluate tag contents and return result in upper case implements {@code 
	 * <uppercase>} tag
	 *
	 * @param node
	 *            current XML parse node
	...
	/**
	 * evaluate tag contents and return result in lower case implements {@code 
	 * <lowercase>} tag
	 *
	 * @param node
	 *            current XML parse node
	...
	/**
	 * evaluate tag contents and capitalize each word. implements {@code 
	 * <formal>} tag
	 *
	 * @param node
	 *            current XML parse node
	...
	/**
	 * evaluate tag contents and capitalize the first word. implements {@code 
	 * <sentence>} tag
	 *
	 * @param node
	 *            current XML parse node
	...
	private static String person(Node node, ParseState ps) {
		...
		if (node.hasChildNodes())
			...
			result = ps.leaf.starBindings.inputStars.star(0); // for <person/>
		result = " " + result + " ";
	...
	private static String person2(Node node, ParseState ps) {
		...
		if (node.hasChildNodes())
			...
			result = ps.leaf.starBindings.inputStars.star(0); // for <person2/>
		result = " " + result + " ";
	...
	private static String learn(Node node, ParseState ps) { // learn, learnf
															// AIML 2.0
		NodeList childList = node.getChildNodes();
		String pattern = "";
		String that = "*";
	...
	/**
	 * implements all 3 forms of the {@code <condition> tag} In AIML 2.0 the
	...
	private static String condition(Node node, ParseState ps) {
		...
		NodeList childList = node.getChildNodes();
		ArrayList<Node> liList = new ArrayList<Node>();
		String predicate = null, varName = null, value = null; // Node p=null,
																// v=null;
		HashSet<String> attributeNames = Utilities.stringSet("name", "var", "value");
		// First check if the <condition> has an attribute "name". If so, get
		// the predicate name.
		...
		if (liList.size() == 0 && (value = getAttributeOrTagValue(node, ps, "value")) != null && predicate != null
				&& ps.chatSession.predicates.get(predicate).equals(value)) {
			return evalTagContent(node, ps, attributeNames);
		} else if (liList.size() == 0 && (value = getAttributeOrTagValue(node, ps, "value")) != null && varName != null
		...
		// otherwise this is a <condition> with <li> items:
			...
			for (int i = 0; i < liList.size() && result.equals(""); i++) {
				...
				if (value != null) {
					...
					if (liPredicate != null && value != null
							&& (ps.chatSession.predicates.get(liPredicate).equals(value)
						...
						return evalTagContent(n, ps, attributeNames);
				} else // this is a terminal <li> with no predicate or value,
						// i.e. the default condition.
					return evalTagContent(n, ps, attributeNames);
			}
		return "";
	...
	private static String recursEval(Node node, ParseState ps) {
		try {
			...
			if (nodeName.equals("#text"))
				...
				return get(node, ps);
			else if (nodeName.equals("map")) // AIML 2.0 -- see also <set> in
												// pattern
				return map(node, ps);
