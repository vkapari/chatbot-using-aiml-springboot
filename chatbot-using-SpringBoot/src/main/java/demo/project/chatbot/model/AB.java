...
public class AB {
	private static final Log log = LogFactory.getLog(AB.class);
	/**
	...
	public static void productivity(int runCompletedCnt, Timer timer) {
		...
		log.info("Completed " + runCompletedCnt + " in " + time + " min. Productivity " + runCompletedCnt / time
				+ " cat/min");
	}
	 ...
	 *
	 * @param bot
	 *            the bot the category belongs to
	 * @param pattern
	...
	public static void saveCategory(Bot bot, String pattern, String template, String filename) {
		...
		if (c.validate()) {
			...
			log.info("Invalid Category " + c.validationMessage);
	}
	 ...
	 *
	 * @param bot
	 *            the bot the category belogs to
	 * @param c
	 *            the category
	 */
	...
	public static void terminalInteraction(Bot bot) {
		...
		// log.info(filteredAtomicCategories.size()+" filtered suggested
		// categories");
	...
	public static void terminalInteractionStep(Bot bot, String request, String textLine, Category c) {
		...
		if (textLine.equals("q"))
			...
			bot.writeQuit();
			/*
			 * Nodemapper udcNode = bot.brain.findNode("*", "*", "*"); if
			 * (udcNode != null) { AIMLSet udcMatches = new
			 * AIMLSet("udcmatches");
			 * udcMatches.addAll(udcNode.category.getMatches());
			 * udcMatches.writeAIMLSet(); }
			 */
			/*
			 * Nodemapper cNode = bot.brain.match("JOE MAKES BEER", "unknown",
			 * "unknown"); if (cNode != null) { AIMLSet cMatches = new
			 * AIMLSet("cmatches");
			 * cMatches.addAll(cNode.category.getMatches());
			 * cMatches.writeAIMLSet(); } if (passed.size() > 0) { AIMLSet
			 * difference = new AIMLSet("difference"); AIMLSet intersection =
			 * new AIMLSet("intersection"); for (String s : passed) if
			 * (testSet.contains(s)) intersection.add(s); passed = intersection;
			 * passed.setName = "passed"; difference.addAll(testSet);
			 * difference.removeAll(passed); difference.writeAIMLSet();
			 * passed.writeAIMLSet(); testSet.writeAIMLSet(); log.info(
			 * "Wrote passed test cases"); }
			 */
			System.exit(0);
		} else if (textLine.equals("skip") || textLine.equals("")) { // skip
																		// this
																		// one
																		// for
																		// now
			skipCategory(bot, c);
		} else if (textLine.equals("s") || textLine.equals("pass")) { // skip
																		// this
																		// one
																		// for
																		// now
